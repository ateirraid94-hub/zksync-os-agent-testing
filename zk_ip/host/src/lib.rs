...
mod test {
    ...
    const ASSET_ID_1: H256 = fixed_bytes!("0xb1f317b7effffcd4e3cf53784ae442ecc4e835c532aaf0e60a046fa8efb96e85").0;  // from log 1
    const ASSET_ID_2: H256 = fixed_bytes!("0xb5eab7cc8c9114c3115a034b49b3d87b0b352aa88c2a9d5ff7339cde105aa44c").0;  // from log 2
    ...
    impl MerkleTree {
        ...
        fn build(&mut self) {
            ...
            for (asset_id, balance) in self.asset_ids.iter().zip(&self.balances) {
                self.layers[0].push(Blake2s256::digest([*asset_id, balance.to_be_bytes()].concat()));
            }
