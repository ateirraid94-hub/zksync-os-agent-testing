...
pub fn ecadd(src: &[u8], mut dst: Vec<u8>) -> Result<(), SubsystemError<Bn254AddErrors>> {
    let allocator = std::alloc::Global;
    let mut resource = <BaseResources<DecreasingNative> as Resource>::FORMAL_INFINITE;
    Bn254AddImpl::execute(&src.as_slice(), &mut dst, &mut resource, allocator)
}

pub fn sha256(src: &[u8], mut dst: Vec<u8>) -> Result<(), SubsystemError<Sha256Errors>> {
    let allocator = std::alloc::Global;
    let mut resource = <BaseResources<DecreasingNative> as Resource>::FORMAL_INFINITE;
    Sha256Impl::execute(&src.as_slice(), &mut dst, &mut resource, allocator)
}