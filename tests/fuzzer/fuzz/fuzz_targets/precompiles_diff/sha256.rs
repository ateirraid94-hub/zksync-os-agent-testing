...
use fuzz_precompiles_forward::precompiles::sha256 as sha256_forward;
use fuzz_precompiles_proving::precompiles::sha256 as sha256_proving;
use revm_precompile::hash;

const SHA256_SRC_REQUIRED_LENGTH: usize = 32;
...
fn fuzz(data: &[u8]) {
    ...
    let r_reth = hash::sha256_run(&src.as_slice()[0..n], u64::MAX);
    let reth_ok = r_reth.as_ref().is_ok_and(|x| !x.reverted);

    let r1 = sha256_forward(&src.as_slice()[0..n], &mut dst1);
    let r1_ok = r1.is_ok();

    let r2 = sha256_proving(&src.as_slice()[0..n], &mut dst2);
    let r2_ok = r2.is_ok();
